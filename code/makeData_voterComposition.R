...
df_raw <- read_excel(path = f, sheet=1) %>% clean_names() |> 
  ...
  mutate(lans_kommun_kod=sprintf("%04d", lans_kommun_kod))
...
# ------------------------------------------------------------------------
# 2026 forecast voter composition, for now, using 2022
# ------------------------------------------------------------------------
yr <- 2026

demo_props_keep <- fread(file.path(getwd(), "input_data", paste0(2022, "_clean_voter.csv") ))
fwrite(demo_props_keep, file.path(getwd(), "input_data", paste0(yr, "_clean_voter.csv") ))
